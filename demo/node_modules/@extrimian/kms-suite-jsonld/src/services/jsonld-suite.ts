...
import { AuthenticationPurpose, DIDDocument, Purpose } from "@extrimian/did-core";
...
export abstract class JsonLDSuite implements IVCSuite {
    ...
    customDocLoader = async (url: string): Promise<any> => {
        if (url.indexOf('did:') > -1 && url.indexOf("#") > -1) {
            const did = url.substring(0, url.indexOf("#"));
            const vmId = url.substring(url.indexOf("#") + 1);

            const didDocument = await this.cacheGetter(did, async () => await this.didDocumentResolver(did));

            const vm = didDocument.verificationMethod.find(x => x.id == url);

            if (vm) {
                return {
                    contextUrl: null, // this is for a context via a link header
                    document: vm, // this is the actual document that was loaded
            ...
            for (const field in didDocument) {
                if (isArray(didDocument[field])) {
                    for (const vm in didDocument[field]) {
                        if (didDocument[field][vm].id == url) {
                            return {
                                contextUrl: null, // this is for a context via a link header
                                document: didDocument[field][vm], // this is the actual document that was loaded
                                documentUrl: url // this is the actual context URL after redirects
                            };
                        }
                    }
